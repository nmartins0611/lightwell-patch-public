...
BUILDER_HOST="${BUILDER_HOST:-builder.example.com}"
MODE="${1:-cicd}"
...
curl -s -X POST "http://${EDA_HOST}:${EDA_PORT}/endpoint" \
  ...
  -d "{
    ...
    \"app_repo\": \"http://${BUILDER_HOST}:3000/demo-admin/config-service\",
    \"target_branch\": \"main\",
    \"remediation_mode\": \"${MODE}\",
    \"lightwell_index\": \"http://${BUILDER_HOST}:8081/simple/pyyaml/\",
